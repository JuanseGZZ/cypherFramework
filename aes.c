...
void Aes256DecryptBlock(unsigned char state[16], roundKey keys[15]);
...
int main(int argc,char *argv[]){
    ...
    /* deciframos */
    Aes256DecryptBlock(state,keys);
    printf("Unencrypted block:\n");
    printKey(state, 16);
...
void InvShiftRows(unsigned char state[16]) {
    ...
    /* Fila 1: [1,5,9,13] rotar derecha 1 */
    tmp       = state[13];
    state[13] = state[9];
    state[9]  = state[5];
    state[5]  = state[1];
    ...
    /* Fila 2: [2,6,10,14] rotar derecha 2 */
    tmp        = state[2];
    state[2]   = state[10];
    state[10]  = tmp;
    ...
    /* Fila 3: [3,7,11,15] rotar derecha 3 = izquierda 1 */
    tmp       = state[3];
    state[3]  = state[7];
    state[7]  = state[11];
    state[11] = state[15];
...
void InvMixColumns(unsigned char state[16]) {
    ...
    for (c = 0; c < 4; c++) {
        ...
        state[base+0] = mul(s0,0x0E) ^ mul(s1,0x0B) ^ mul(s2,0x0D) ^ mul(s3,0x09);
        state[base+1] = mul(s0,0x09) ^ mul(s1,0x0E) ^ mul(s2,0x0B) ^ mul(s3,0x0D);
        state[base+2] = mul(s0,0x0D) ^ mul(s1,0x09) ^ mul(s2,0x0E) ^ mul(s3,0x0B);
        state[base+3] = mul(s0,0x0B) ^ mul(s1,0x0D) ^ mul(s2,0x09) ^ mul(s3,0x0E);
    }
}
void Aes256DecryptBlock(unsigned char state[16], roundKey keys[15]) {
    int r;
